...
namespace FuncationTest
{
    public class PicturesInfo
    {
        ...
        public PicturesInfo(int row, int col, Byte[] pictureData)
        {
            this.Row = row;
            this.Col = col;
            ...
            if (null != shapeContainer)
            {
                ...
                foreach (var shape in shapeList)
                {
                    if (shape is HSSFPicture && shape.Anchor is HSSFClientAnchor)
                    {
                        ...
                        {
                            picturesInfoList.Add(new PicturesInfo( anchor.Row1, anchor.Col2, picture.PictureData.Data));
                        }
                    }
                }
            }
